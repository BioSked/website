...
assert.equal(snapshot.sourceHost, 'kb.biosked.com', 'KB snapshot must come from the BioSked HubSpot domain');
...
for (const article of snapshot.articles) {
  ...
  assert.equal(article.previewPath, `/kb-preview${article.sourcePath}`, `bad preview path for ${article.sourcePath}`);
  ...
  assert.doesNotMatch(article.bodyHtml, /https:\/\/kb\.biosked\.com\/(?:en|fr)\/knowledge\//i, `unrewritten internal link in ${article.sourcePath}`);
...
const requiredFrontendFiles = [
  ...
  'src/pages/kb-preview/index.astro',
  'src/pages/kb-preview/search-index.json.ts',
  'src/pages/kb-preview/[lang]/knowledge/index.astro',
  'src/pages/kb-preview/[lang]/knowledge/kb-tickets/new.astro',
  'src/pages/kb-preview/[lang]/knowledge/[...slug].astro',
];
...
assert.match(
  ...
  'KB language links must remain inside the preview during automatic locale selection',
);
